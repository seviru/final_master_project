...
clusters_info_retriever_bigger10.py: It's an archaic version of "cluster_info_retriever.py" and "cluster_finder.py",
		Including both scripts in the same place and storing information in a different way.
		USAGE: $python clusters_info_retriever_bigger10.py
		NEEDED: Need Python 3.7. If It fails, try: $module load Python/3.7.2-GCCcore-8.2.0

<OUTFILE>.txt: We will have a .txt file in this folder containing the clusters list which meet the requirements of
		the "cluster_finder.py" script.

./fastas/: Folder where we will store the FASTA (.fas) files of all the genes on each cluster from <OUTFILE>.txt .

./tables/: Folder where we will store the TABLE (.tsv) files of all the genes on each cluster from <OUTFILE>.txt .
